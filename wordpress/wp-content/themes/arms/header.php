...
<html>
  ...
  <body class="<?= wp_title() ?> md:h-screen md:overflow-y-hidden">
    <header
      class="fixed h-48 md:h-1/4 flex flex-row items-center justify-between px-12 sm:px-24 left-0 top-0 right-0 z-20 pointer-events-none <?=$dark ? 'dark' : 'light' ?>"
    >
      <div>
        ...
        <a href="/<?= $home_section ? '#'.$home_section : '' ?>" class="cursor-pointer pointer-events-auto mt-5 hidden md:block" id="page-back-nagivation">
          <svg
          xmlns="http://www.w3.org/2000/svg"
          width="53"
          height="16"
      ...
      <div class="flex flex-row gap-4 items-center relative md:pr-12">
        <p class="text-2xl <?= $dark ? '' : 'text-white' ?>"><?= $page_indicator ?></p>
